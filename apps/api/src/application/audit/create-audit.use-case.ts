...
export class CreateAuditUseCase {
  ...
  async execute(rawUrl: string): Promise<CreateAuditResult> {
    ...
    const { discovery, crawl, inventory, analysis, entity } = await this.executeAuditUseCase.execute(
      audit.id,
      url.value,
    );
    const completedAudit = await this.auditRepository.markCompleted(audit.id, new Date());

    return { audit: completedAudit, discovery, crawl, inventory, analysis, entity };
  }
}
