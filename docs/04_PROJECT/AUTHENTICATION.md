...
- ~~**Gating existing routes behind login.**~~ **Superseded at `F9-S03`** (`docs/04_PROJECT/DECISION_LOG.md#cto-093`): `app/(shell)/layout.tsx` now redirects to `/login` if `getCurrentUser()` returns null. At the time of `F9-S02`, the ticket's own TECHNICAL section argued against retrofitting an auth check onto existing routes — that reasoning applied to *this* ticket's scope, not permanently; `F9-S03`'s explicit "Authenticated Application Shell" objective is what changed it.
...
- ~~**A `/logout` route.**~~ **Superseded at `F9-S03`**: `logoutUser()` (`apps/web/app/auth-actions.ts`) deletes the session cookie, wired to the new Top Bar User Menu's "Sign Out."
- ~~**Showing signed-in state in `AppHeader`.**~~ **Superseded at `F9-S03`**: the new authenticated shell's Top Bar reads `getCurrentUser()` and shows the signed-in user's name/initials/email in its User Menu. `AppHeader` itself (the pre-shell minimal bar, still used by auth pages) still shows no signed-in state — unrequested there.
