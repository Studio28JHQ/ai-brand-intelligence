...
- **Two distinct chrome systems**, chosen client-side by `usePathname()` in `AppHeader` (root layout) — not by separate root layouts, so URLs never had to move. `/` gets `MarketingHeader`; the authenticated application shell (`/workspace` and everything under it — `/clients`, `/projects`, `/audits`, `/optimization`, `/reports`, `/settings`) gets `AppShell` (Sidebar + TopBar, `app/(shell)/layout.tsx`, `F9-S03` — see `docs/04_PROJECT/DECISION_LOG.md#cto-093`), which `AppHeader` renders nothing for to avoid a duplicate header; every other route (auth pages) keeps the original minimal internal brand bar.
- **Breadcrumbs on every non-home internal screen**, always ending in the current page (not a link) and starting from "Dashboard" (`/workspace` — renamed from "Workspace" at `F9-S03` to match the Sidebar's own naming; the route itself is unchanged). Dashboard shows `Dashboard > {Project}`; Campaign/Consultant/Report show `Dashboard > Project Dashboard > {Page}` (the per-Project Dashboard is labeled "Project Dashboard" in this chain specifically, to stay distinct from the hub-level "Dashboard" crumb before it); Audit Detail shows `Dashboard > Audit`.
...
Delivered at `F9-S03` (see `docs/04_PROJECT/DECISION_LOG.md#cto-093`) — a Left Sidebar + Top Bar shell (`app/components/shell/{AppShell,Sidebar,TopBar}.tsx`) wrapping everything under `app/(shell)/` (a Next.js route group — adds no URL segment, so every pre-existing link/bookmark to `/workspace`, `/projects/:id/dashboard`, `/audits/:id`, etc. still works unchanged).

- **Real authentication gating, for the first time**: `app/(shell)/layout.tsx` calls `getCurrentUser()` server-side and `redirect('/login')` if there's no session — superseding `F9-S02`'s "Rejected Scope: Gating existing routes behind login." Every route under the shell now requires a real session; the marketing site and auth pages remain open.
- **Dashboard redesign, not a restyle**: the former `/workspace` page mixed Client/Project/Audit CRUD, a raw new-Audit-result inspector, and the AI Daily Briefing into one long scroll — explicitly what `F9-S03`'s "Do not display large audit tables" rules out for the first screen. That management UI moved to new dedicated pages (`/clients`, `/projects`, `/audits`, `/optimization`, `/reports`); `/workspace` itself is now a pure Server Component that groups the same, already-computed `BriefingModel` (`F7-S04`) into named sections — Priority Cards (`project-attention`/`ai-visibility-regression`/`critical-finding`), Pending Actions (`campaign-awaiting-verification`), AI Recommendations (`high-impact-opportunity`), Recent Activity (`recent-improvement`) — plus a Latest Reports section (Projects whose current Cycle has reached `verification`/`completed`). No new data was invented; this is the same Daily Briefing data, restructured for scanability.
- **Honest "Workspace Selector" and "Agency Selector"**: this platform has no multi-workspace or multi-agency data model — every `Client`/`Project`/`User` belongs to one implicit agency. Both are real, working dropdown components (not static text), each showing its one real option, rather than a static label or a fabricated switcher pretending multiple exist. The Agency Selector lives at the top of the Sidebar; the Workspace Selector is the first Top Bar control.
- **Notifications are real data, not a mock feed**: the bell in the Top Bar surfaces the same `BriefingItem[]` the Dashboard uses (fetched once per request in the shell layout, passed down as a prop), badge-counted by `confidence === 'high'`, each linking to its Project's Dashboard.
- **Search is real, not a stub**: the Top Bar search box submits to `/clients?q=<term>`, which server-side-filters the real Client list by name — there is no fabricated "search everything" index.
- **Settings is honest about what doesn't exist yet**: Account Settings shows real data from `GET /auth/me`; Workspace Settings is an explicit `EmptyState` ("Workspace-level settings (agency branding, team members, billing) aren't part of this platform yet") rather than empty toggles that do nothing.
- **A real Sign Out**: `logoutUser()` (`auth-actions.ts`) deletes the session cookie — superseding `F9-S02`'s "Rejected Scope: A `/logout` route." No API call is needed since sessions are stateless JWTs (`CTO-084`); they simply expire.
- **Responsive**: the Sidebar becomes a slide-in overlay (with a click-to-dismiss scrim) below 900px, toggled by a Top Bar hamburger button; the Top Bar search box hides at that width. Verified live via the browser tool's `resize_window` (unlike `F9-S01`'s verification, this did propagate to the tab's actual rendered viewport in this session).
...
- **Fabricated multi-agency/multi-workspace switching** (`F9-S03`). No such data model exists; see "Authenticated Application Shell" above for what was built instead.
- **A real notifications backend** (persisted, dismissible, read/unread state). The Top Bar bell reuses the existing Daily Briefing computation; a genuine notifications system (storage, read state, push) is a larger capability this ticket's ACCEPTANCE CRITERIA didn't ask for.
- **Functional Workspace/Account Settings beyond what already exists**. Only `GET /auth/me`-backed profile display was built; editing profile fields, team members, billing, and agency branding have no backing endpoints yet.
