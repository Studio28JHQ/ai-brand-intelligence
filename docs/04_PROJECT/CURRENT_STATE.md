...
- **F2 — Business Engines**: Discovery, Crawler, Inventory, Analysis (with the generic Rule Engine), Entity, Knowledge Graph, and AI Visibility engines implemented as independent, framework-agnostic modules under `services/*`. Severity classification folded into the Analysis Engine rather than a standalone Classification Engine. Recommendation generation implemented as a Presentation-layer function (`apps/api/src/presentation/audit/recommendation.service.ts`), not persisted, not a Business Engine.
