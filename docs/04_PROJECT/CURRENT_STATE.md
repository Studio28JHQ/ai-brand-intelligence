...
- **Conversation Session** / **Conversation Request** — what a caller provides: a `ConversationSession` (`sessionId`, `projectId`, `startedAt` — a caller-supplied value object, never persisted server-side; `createConversationSession(projectId)` is a convenience constructor) plus a `UserIntent` (`type` from a fixed `ConversationIntentType` enum, and a `question`).
...
- **Provider abstraction** — `AiProvider` (`complete(request): Promise<AiResponse>`) is the port; `NoOpAiProvider` (`apps/api/src/infrastructure/ai-conversation/`) is its only implementation today — not OpenAI, Anthropic, or Gemini, just a deterministic `{ status: 'unavailable', content: null }` Null Object, present so the whole pipeline is genuinely exercisable end-to-end before a real provider exists. Wiring a real provider later means adding one class and rebinding `AI_PROVIDER`; the Orchestrator itself does not change.
- **AI Response** / **Conversation Response** — what a Provider returns (`AiResponse`) is wrapped, alongside the session id, into the `ConversationResponse` the Orchestrator ultimately returns.

# Current Phase

F7 — AI Consultant, in progress. F6 — Pilot Readiness fully delivered (`F6-S01` through `F6-S07`). AI Context Builder (`F7-S01`) and AI Conversation Orchestrator (`F7-S02`) delivered.
