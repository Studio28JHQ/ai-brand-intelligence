...
- **F6 — Pilot Readiness**: Client aggregate (`F6-S01`) — the top-level entity that owns Projects, enabling one workspace to serve multiple customers. Executive Dashboard (`F6-S02`) — a per-Project read view (`GET /projects/:id/dashboard`) composed entirely from existing Read Models: Project/Client/Audit repositories, the Finding/AI Visibility/Baseline History read repositories, and the Recommendation Engine. Recommendation Engine (`F6-S03`) — `apps/api/src/application/recommendation/`: recommendations are projections derived from Findings and Project/Audit context (never persisted), with a deterministic prioritization model (expected impact, estimated effort, confidence) producing each Recommendation's priority; exposed through the Audit response and the Dashboard's Priority Actions. See `docs/03_PRODUCT/FUTURE_ROADMAP.md` for full sprint detail.
...
F6 — Pilot Readiness, in progress. Client aggregate (`F6-S01`), Executive Dashboard (`F6-S02`), and Recommendation Engine (`F6-S03`) delivered.
